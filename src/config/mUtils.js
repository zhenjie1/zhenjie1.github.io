...
import { rescueCollectionTime, rescueUploadTime} from './url'
// console.log(ss)
/**
 * 存储localStorage
 */
...
export const titleConfig = (to) => {
	...
	if (store.state.userInfo && store.state.userInfo.userType != 3 && path === "/rescue/task") title.returnIsShow = false
...
var setInterUploadFn, setInterSendPointFn, rescusPositioonPoint_list = [];
export const setInterUploadLatitudeLongitude = function (id) {
	uploadLatitudeLongitude(id, rescusPositioonPoint_list)
	setInterUploadFn = setInterval(() => uploadLatitudeLongitude(id, rescusPositioonPoint_list), rescueCollectionTime);

	socketPushLngLat()
	setInterval(() => socketPushLngLat(), rescueUploadTime)
}

function socketPushLngLat() {
...
function uploadLatitudeLongitude(id, pointSaveArr) {
	...
	// console.log(parseFloat(latLon.Latitude) < parseFloat(latLon.Longitude))

	if (parseFloat(latLon.Latitude) < parseFloat(latLon.Longitude)) {
		[latLon.latitude, latLon.longitude] = [latLon.longitude, latLon.latitude]
	}
	pointSaveArr.push({
